...
const saveDrawing = () => {
	const confirmed = confirm(
		'Do you want to post your drawing in the drawings gallery?'
	)
	if (confirmed) {
		fetch('https://drawings-backend.vercel.app/api/postDrawing', {
			...
			.then(() => {
				document
					.getElementById('upload-success-toast')
			...
			.catch((err) => {
				document
					.getElementById('upload-fail-toast')
					.classList.remove('inactive')
				setTimeout(
					() =>
						document
...
const draw = (event) => {
	...
	if (isDrawing) {
		const x = event.targetTouches
			? event.targetTouches[0].pageX - canvas.offsetLeft
			: event.offsetX
		const y = event.targetTouches
			? event.targetTouches[0].pageY - canvas.offsetTop
			: event.offsetY
		drawLine(x, y)
		p1X = x
