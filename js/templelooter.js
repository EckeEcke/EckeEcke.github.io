...
function movePlayer() {
	...
	if (
		buttonsPressed.d &&
		player1.x < (9 * canvasWidth) / 10 &&
		!player1.isCollision.x
	) {
		player1.x += 2
	}
...
function changeTouchPosition(event) {
	...
	touchControls.x = event.targetTouches
		? event.targetTouches[0].pageX - overlay.offsetLeft
		: event.offsetX
	touchControls.y = event.targetTouches
		? event.targetTouches[0].pageY - overlay.offsetTop
		: event.offsetY
}
...
function movePlayerGamepad() {
	...
	if (
		navigator.getGamepads()[0].buttons[13].pressed &&
		player1.y < (9 * canvasHeight) / 10
	) {
		player1.y += 2
	}
}
...
function collision() {
	if (
		enemies.obstacle.x < player1.x + player1.width &&
		enemies.obstacle.x >= player1.x + player1.width - 4
	) {
		if (
			player1.y < enemies.obstacle.y + enemies.obstacle.height - 10 ||
			enemies.obstacle.y2 < player1.y + player1.height
		) {
	...
	if (
		player1.x < enemies.obstacle.x + enemies.obstacle.width &&
		player1.x + player1.width > enemies.obstacle.x
	) {
		if (
			player1.y <= enemies.obstacle.y + enemies.obstacle.height &&
			player1.y >= enemies.obstacle.y + enemies.obstacle.height - 2
		) {
		...
		if (
			player1.y < enemies.obstacle.y2 + 2 &&
			player1.y + player1.height > enemies.obstacle.y2
		) {
			player1.isCollision.y = true
			player1.kickback.y = -4
		}
...
function collectKey() {
	if (
		player1.x < items.key.x + 40 &&
		player1.x + player1.width > items.key.x &&
		player1.y < items.key.y + 25 &&
		player1.y + player1.height > items.key.y &&
		!items.key.collected
	) {
...
function collectTreasure() {
	if (
		player1.x < items.treasure.x + items.treasure.width &&
		player1.x + player1.width > items.treasure.x &&
		player1.y < items.treasure.y + items.treasure.height &&
		player1.y + player1.height > items.treasure.y &&
...
function batCollision(bat) {
	if (
		player1.x + 15 < bat.x + 35 &&
		player1.x + player1.width > bat.x + 15 &&
		player1.y + 10 <= bat.y + 20 &&
		player1.y + player1.height >= bat.y &&
		!player1.invincibility
	) {
		playSound(sounds.hurt)
...
function toggleBackground() {
	...
	gameState.backgroundLavaLevel =
		gameState.background1Lava === true
			? sprites.backgrounds.lava1
			: sprites.backgrounds.lava2
}

function endGame() {
	if (
		player1.x + player1.width < -30 ||
		player1.lives === 0 ||
		gameState.timer <= 0
	) {
		player1.lives = 0
...
function moveBridges() {
	items.bridge1.y < canvasHeight + 200
		? (items.bridge1.y += 1)
		: (items.bridge1.y = -200)
...
function lavaCollision() {
	if (
		(player1.x > 60 && !player1.carriedByBridge && player1.x < 260) ||
		player1.y > canvasHeight + 20 ||
		player1.y < -20
	) {
		player1.lives -= 1
...
function bridgeCollision(bridge, moveY, secondBridgeY) {
	if (
		bridge.x < player1.x + player1.width &&
		bridge.x + bridge.width > player1.x
	) {
		if (
			(player1.y + player1.height - 10 < bridge.y + bridge.height - 10 &&
				bridge.y < player1.y + player1.height) ||
			(player1.y + player1.height - 10 <
				bridge.y + secondBridgeY + bridge.height - 10 &&
				bridge.y + secondBridgeY < player1.y + player1.height)
		) {
	...
	if (
		bridge.x < player1.x + player1.width / 2 &&
		bridge.x + bridge.width > player1.x + player1.width / 2
	) {
		player1.y += moveY
	}
}
...
function drawBridges() {
	ctx.drawImage(
		sprites.items.bridge,
		items.bridge1.x,
		items.bridge1.y,
		items.bridge1.width,
		items.bridge1.height
	)
	ctx.drawImage(
		sprites.items.bridge,
		items.bridge1.x,
...
function drawTreasure() {
	if (!items.treasure.collected) {
		disableImageSmoothing()
		ctx.drawImage(
			sprites.items.treasure,
			items.treasure.x,
			items.treasure.y,
			items.treasure.width,
			items.treasure.height
		)
	}
}
...
function drawScoreBoard() {
	...
	ctx.textAlign = 'start'
	const text =
		'Score ' + String(player1.score).padStart(4, '0') + ' 🗝x' + player1.keys
...
function drawWall() {
	...
	ctx.drawImage(
		...
		enemies.obstacle.x,
		0,
		enemies.obstacle.width,
		enemies.obstacle.height
	)
	ctx.drawImage(
		sprites.items.wall,
		enemies.obstacle.x,
