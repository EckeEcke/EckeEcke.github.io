...
const itemCollision = () => {
	if (collision(item) && !gameState.itemCollected) {
		...
		setTimeout(replaceItem, 1000)

		if (gameState.score % 5 === 0) {
			gameState.level += 1
			const levelData = levels.find(
				(levelObject) => levelObject.level === gameState.level
			)
			if (!levelData) return
...
const collision = (element) => {
	const snakeHead = snake[0]
	return (
		element.x + 0.5 > snakeHead.x &&
		element.x < snakeHead.x + 0.5 &&
		element.y + 0.5 > snakeHead.y &&
		element.y < snakeHead.y + 0.5
	)
}
...
const drawElements = (height) => {
	...
	orderedElements.forEach((element) => {
		if (element.type === 'snake') {
			iso.add(
				Shape.Prism(new Point(0 + element.x, element.y, 0), 0.5, 0.5, height),
				gameState.activePalette.snakeColor
			)
			...
			const isHead =
				snake.findIndex(
					(part) => part.x === element.x && part.y === element.y
				) === 0
			const currentMove = isHead
				? gameState.direction
				: element.moves && element.moves[0]
			...
			if (element.type === 'poop' && element.x >= 0) {
				iso.add(
					Shape.Pyramid(new Point(poop.x, poop.y, 0), 0.5, 0.5, 0.5),
					gameState.activePalette.poopColor
				)
			} else if (element.type === 'item' && !gameState.itemCollected) {
				iso.add(
					Shape.Prism(new Point(0 + element.x, element.y, 0), 0.5, 0.5, 0.5),
					gameState.activePalette.itemColor
				)
				iso.add(
					Shape.extrude(
						new Path([
