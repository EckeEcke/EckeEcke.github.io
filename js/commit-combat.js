...
const GH_MONO = ['ui-monospace', 'SFMono-Regular', 'SF Mono', 'Menlo', 'Consolas', 'monospace']
...
const setupLauncher = () => {
    ...
    radioContainer.style.cssText = 'display:flex; justify-content:space-around; padding:10px; background:#22272e; border-radius:4px'
...
const initGame = async (p1, p2, mode, battleData) => {
    ...
    const loadAvatar = async (cont, name, isEnemy) => {
        ...
        try {
            ...
            const letter = createSharpText(name[0].toUpperCase(), 14, true, isEnemy ? '#ff4444' : '#39d353')
