// Github fonts to mimic github

const GH_SANS = ['-apple-system', 'BlinkMacSystemFont', '"Segoe UI"', 'Helvetica', 'Arial', 'sans-serif', '"Apple Color Emoji"', '"Segoe UI Emoji"']
const GH_MONO = ['ui-monospace', 'SFMono-Regular', 'SF Mono', 'Menlo', 'Consolas', '"Liberation Mono"', 'monospace']
...
const setupLauncher = () => {
    ...
    input1.style.cssText = 'padding:8px; border-radius:4px; border:1px solid #444c56; background:#22272e; color:white'
    
    const input2 = document.createElement('input')
    input2.placeholder = 'Enter github user name'
    input2.style.cssText = 'padding:8px; border-radius:4px; border:1px solid #444c56; background:#22272e; color:white'

    const radioContainer = document.createElement('div')
    radioContainer.style.cssText = 'display:flex; justify-content:space-around; padding:10px 4px 10px 0; background:#22272e; border-radius:4px'
    
    const createRadio = (id, label, checked) => {
        const wrap = document.createElement('label')
        wrap.style.cursor = 'pointer'
        const radio = document.createElement('input')
        radio.style.cssText = 'accent-color: red'
        radio.type = 'radio'; radio.name = 'mode'; radio.value = id; radio.checked = checked
        wrap.append(radio, ' ' + label)
        return wrap
    }
    
    radioContainer.append(createRadio('cpu', '1 player', true), createRadio('pvp', '2 player', false))
    ...
    startBtn.onclick = () => {
        ...
        uiOverlay.remove()
        showPreGameScreen(p1, p2, mode)
    }
}

const showPreGameScreen = async (p1, p2, mode) => {
    ...
    const createSharpText = (text, fontSize, isBold = true) => {
        const t = new PIXI.Text({ 
            text, 
            resolution: res, 
            style: { fontFamily: GH_SANS, fontSize, fontWeight: isBold ? 'bold' : 'normal', fill: '#ffffff', stroke: { color: '#ffffff', width: 0.2 }, align: 'center', wordWrap: true, wordWrapWidth: 120 } 
        })
        t.roundPixels = true; return t
    }
    const drawPreAvatar = async (name, x, y, color) => {
        const container = new PIXI.Container()
        container.position.set(x, y)
        stage.addChild(container)
        const border = new PIXI.Graphics().roundRect(-42, -42, 84, 84, 8).stroke({ width: 3, color })
        ...
        try {
            ...
            const mask = new PIXI.Graphics().roundRect(-40, -40, 80, 80, 6).fill(0xffffff)
            sprite.mask = mask
            container.addChild(mask, sprite)
        } catch(e) {}
        const txt = createSharpText(name, 18)
        txt.anchor.set(0.5)
        txt.y = 65; container.addChild(txt)
    }
    await Promise.all([drawPreAvatar(p1, 100, 150, 0x39d353), drawPreAvatar(p2, 300, 150, 0xff4444)])
...
const initGame = async (p1, p2, mode) => {
    const keys = { a: false, d: false, w: false, arrowleft: false, arrowright: false, arrowdown: false }
    ...
    const DESIGN_H = 400
    const res = Math.max(window.devicePixelRatio, 2)
    await app.init({ background: '#1a1a1a', antialias: true, resizeTo: window, resolution: res, autoDensity: true })
    document.getElementById('game-container').appendChild(app.canvas)
    ...
    window.addEventListener('resize', resize)
    resize()
    const createSharpText = (text, fontSize, isBold = true, color = '#ffffff', isMono = false) => {
        const t = new PIXI.Text({ text, resolution: res, style: { fontFamily: isMono ? GH_MONO : GH_SANS, fontSize, fontWeight: isBold ? 'bold' : 'normal', fill: color, stroke: { color, width: 0.2 }, wordWrap: true, wordWrapWidth: SIDEBAR_W - 10 } })
        t.roundPixels = true; return t
    }
    ...
    const createBullet = (x, y, vy, isEnemy) => {
        if (gameOver) return
        const b = new PIXI.Graphics().circle(0, 0, 4).fill(isEnemy ? 0xff4444 : 0xffff00)
        b.x = x
        b.y = y
        stage.addChild(b)
        const tick = () => {
    ...
    try {
        const res = await fetch(`https://eckeecke.pythonanywhere.com/get-battle/${p1}/${p2}`)
        const data = await res.json()
        const tileSize = (DESIGN_W - 70) / 7
        const drawGrid = (cont, grid, y) => {
            cont.position.set(20, y)
            grid.forEach((row, rIdx) => row.forEach((c, cIdx) => {
                const s = new PIXI.Graphics().roundRect(0,0,tileSize,tileSize,6).fill(getGitColor(c))
                s.x = cIdx*(tileSize+6)
                s.y = rIdx*(tileSize+6)
                s.lives = c
                cont.addChild(s)
        ...
        drawGrid(enemyShields, data.enemy.shield_grid, 50)
        drawGrid(playerShields, data.player.shield_grid, 280)
        const loadAvatar = async (cont, name, isEnemy) => {
            const border = new PIXI.Graphics().roundRect(-13, -13, 26, 26, 4).stroke({ width: 2, color: isEnemy ? 0xff4444 : 0x39d353 })
            ...
            spr.width = 24
            spr.height = 24
            const mask = new PIXI.Graphics().roundRect(-12, -12, 24, 24, 3).fill(0xffffff)
            spr.mask = mask
            cont.addChild(mask, spr)
        }
        await Promise.all([loadAvatar(playerChar, p1, false), loadAvatar(enemyChar, p2, true)])
        if (mode === 'cpu') {
            const shootAI = () => { if(!gameOver){ 
